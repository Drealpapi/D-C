...
export default function SanctumSidebar({ view, setView, bgClass = 'bg-[#0a0b0f]' }: {
  view: AdminView
  ...
  return (
    <aside className={`w-[72px] lg:w-64 shrink-0 ${bgClass} border-r border-white/[0.05] min-h-screen flex flex-col transition-colors duration-300`}>

      {/* Brand */}
      <div className="px-4 py-5 border-b border-white/[0.05]">
        <div className="flex items-center gap-3">
          <ThemeLogo size={30} forceDark />
          <div className="hidden lg:block">
            <p className="font-serif text-sm text-white/90 tracking-wide leading-none">Divine Couture</p>
            <p className="text-[10px] tracking-[0.18em] text-white/30 uppercase mt-0.5">Sanctum Admin</p>
          </div>
        </div>
      </div>
      ...
      {/* Admin badge */}
      {currentAdmin && (
        <div className="hidden lg:block px-4 py-3 border-b border-white/[0.05]">
          <div className="flex items-center gap-2.5">
            ...
            <div className="min-w-0">
              <p className="text-xs font-medium text-white/80 truncate">{currentAdmin.name}</p>
              <p className="text-[10px] text-white/30 truncate">{currentAdmin.email}</p>
            </div>
          </div>
        </div>
      )}
      ...
      <nav className="flex-1 px-2 py-4 space-y-5 overflow-y-auto">
        {NAV_GROUPS.map((group) => (
          <div key={group.label}>
            <p className="hidden lg:block text-[10px] uppercase tracking-[0.2em] text-white/20 px-3 mb-2">
              {group.label}
            </p>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="hidden lg:block text-left flex-1 min-w-0">
                      <p className={`text-sm font-medium leading-none ${active ? 'text-stone-900' : ''}`}>{label}</p>
                      <p className={`text-[10px] mt-0.5 ${active ? 'text-stone-700' : 'text-white/30'}`}>{desc}</p>
                    </div>
      ...
      <div className="px-2 py-4 border-t border-white/[0.05] space-y-0.5">
        ...
        <Link
          href="/"
          target="_blank"
          className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm text-white/40 hover:bg-white/[0.05] hover:text-white/80 transition"
        >
        ...
        <button
          onClick={logout}
          className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm text-white/40 hover:bg-red-500/10 hover:text-red-400 transition"
        >
