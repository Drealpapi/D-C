...
export default function ProfileView() {
  ...
  return (
    <div className="bg-[#faf8f5] dark:bg-[#0d0f1a] min-h-screen transition-colors duration-300">
      <div className="max-w-3xl mx-auto px-4 md:px-8 py-12 md:py-16 space-y-5">
        ...
        <div className="bg-white dark:bg-white/[0.03] border border-stone-100 dark:border-white/[0.06] rounded-2xl px-6 py-8 flex items-center gap-5 shadow-sm">
          <div className="w-16 h-16 rounded-full bg-gradient-to-br from-amber-400 to-amber-600 flex items-center justify-center shrink-0 shadow-md">
            <span className="font-serif text-2xl text-white font-bold select-none">
              {user.name.charAt(0)}
            </span>
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-[11px] uppercase tracking-[0.2em] text-amber-600 dark:text-amber-400 mb-1">Welcome back</p>
          ...
          <Link
            href="/shop/earrings"
            className="hidden sm:flex items-center gap-2 px-4 py-2 bg-amber-500 text-stone-900 text-xs font-bold tracking-wider uppercase hover:bg-amber-400 transition rounded-sm shrink-0"
          >
        ...
        <div className="grid grid-cols-3 gap-3">
          {quickLinks.map(({ label, sub, icon: Icon, href }) => (
            <a
              key={label}
              href={href}
              className="group bg-white dark:bg-white/[0.03] border border-stone-100 dark:border-white/[0.06] rounded-xl px-4 py-5 flex flex-col items-center text-center gap-2.5 hover:border-amber-200 dark:hover:border-amber-500/30 hover:shadow-md transition duration-200"
            >
              <Icon className="w-5 h-5 text-stone-400 dark:text-stone-500 group-hover:text-amber-600 dark:group-hover:text-amber-400 transition" />
              <div>
        ...
        {/* ── Order history ── */}
        <div className="bg-white dark:bg-white/[0.03] border border-stone-100 dark:border-white/[0.06] rounded-2xl overflow-hidden shadow-sm">
          <div className="px-6 py-4 border-b border-stone-50 dark:border-white/[0.04] flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              <Package className="w-4 h-4 text-amber-500" />
              <h2 className="font-serif text-lg text-stone-900 dark:text-stone-100">Order History</h2>
            </div>
            <span className="text-xs text-stone-400 dark:text-stone-500 bg-stone-50 dark:bg-white/[0.04] px-2.5 py-1 rounded-full">
          ...
          <div className="divide-y divide-stone-50 dark:divide-white/[0.04]">
            {DEMO_ORDERS.map((order) => (
              <div key={order.id} className="px-6 py-4 flex items-center justify-between gap-4 hover:bg-stone-50/60 dark:hover:bg-white/[0.02] transition duration-150">
        ...
        {/* ── Account details ── */}
        <div className="bg-white dark:bg-white/[0.03] border border-stone-100 dark:border-white/[0.06] rounded-2xl overflow-hidden shadow-sm">
          <div className="px-6 py-4 border-b border-stone-50 dark:border-white/[0.04] flex items-center gap-2.5">
              ...
              <div key={label} className="px-6 py-4 flex items-center justify-between">
                ...
                {label === 'Password' && (
                  <Link
                    ...
                    className="flex items-center gap-1.5 text-xs text-stone-400 dark:text-stone-500 hover:text-amber-600 dark:hover:text-amber-400 transition"
                  >
