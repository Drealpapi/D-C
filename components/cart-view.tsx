...
export default function CartView() {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 md:px-8 py-10 md:py-14">
      <h1 className="font-serif text-3xl md:text-4xl text-stone-900 dark:text-stone-100 mb-10">Your Bag</h1>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        <div className="lg:col-span-2 space-y-6">
          {items.map((item) => (
            <div key={`${item.id}-${item.selectedSize}`} className="flex gap-4 pb-6 border-b border-stone-100 dark:border-stone-800">
              <Link href={`/product/${item.id}`} className="relative w-24 h-24 flex-shrink-0 bg-stone-100 dark:bg-stone-800 overflow-hidden">
                <Image src={item.image} alt={item.name} fill className="object-cover object-center" sizes="96px" />
              </Link>
              <div className="flex-1 min-w-0">
                <div className="flex justify-between gap-2">
                  <div>
                    <p className="text-xs text-stone-400 dark:text-stone-500 uppercase tracking-wider mb-0.5 capitalize">{item.category}</p>
                    <Link href={`/product/${item.id}`} className="text-sm font-medium text-stone-800 dark:text-stone-200 hover:text-stone-600 dark:hover:text-stone-400 transition line-clamp-2">{item.name}</Link>
                    {item.selectedSize && <p className="text-xs text-stone-400 dark:text-stone-500 mt-1">Size: {item.selectedSize}</p>}
                  </div>
                  <button onClick={() => removeItem(item.id)} className="text-stone-300 dark:text-stone-600 hover:text-stone-600 dark:hover:text-stone-300 transition flex-shrink-0" aria-label="Remove item">
                    <Trash2 className="w-4 h-4" />
                  </button>
                </div>
                <div className="flex items-center justify-between mt-4">
                  <div className="flex items-center border border-stone-200 dark:border-stone-700">
                    <button onClick={() => updateQuantity(item.id, item.quantity - 1)} className="px-3 py-1.5 text-stone-500 dark:text-stone-400 hover:text-stone-900 dark:hover:text-stone-100 transition" aria-label="Decrease quantity"><Minus className="w-3 h-3" /></button>
                    <span className="px-3 py-1.5 text-sm text-stone-800 dark:text-stone-200 min-w-[2rem] text-center">{item.quantity}</span>
                    <button onClick={() => updateQuantity(item.id, item.quantity + 1)} className="px-3 py-1.5 text-stone-500 dark:text-stone-400 hover:text-stone-900 dark:hover:text-stone-100 transition" aria-label="Increase quantity"><Plus className="w-3 h-3" /></button>
                  </div>
                  <span className="text-sm font-semibold text-stone-900 dark:text-stone-100">£{(item.price * item.quantity).toFixed(2)}</span>
                </div>
              </div>
            </div>
          ))}
